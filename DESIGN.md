...
- **Signal Cobalt:** 링크 상태, 프로젝트 무대와 소개 문구의 핵심 문장에 사용한다. 첫 화면의 풍경은 차가운 중성색을 유지한다.
...
- **Silver Field:** 소개, 소개 문구와 연락 구간의 밝은 반전 표면이다. 소개 문구는 제목과 장식 없이 왼쪽 정렬하고, 이름과 직무는 두 줄로 배치한다.
...
첫 구간에서는 로고만 표시하고 현재 위치는 aria-current로 전달한다. 같은 페이지의 이동은 아래 화살표, 내부 문서는 오른쪽 화살표, 외부 링크는 오른쪽 위 화살표를 사용하며 새 탭 안내는 스크린 리더에만 제공한다. 실험실은 Always building. 제목만 표시하고 보조 설명은 두지 않는다. 이메일 실행에는 봉투 아이콘을 사용한다. 모바일에서 숨겨진 실험실 미리보기의 선택 상태를 링크에 남기지 않는다.
...
등장은 첫 화면 인스턴스에서 한 번 재생한다. PC 2.6초, 터치 기기 1.8초 동안 높은 시점의 윤곽선에서 재질이 위에서 아래로 드러나며 카메라가 기본 위치로 내려온다. 주변의 가벼운 삼각 연결망은 등장 후 숨기고 숫자는 표시하지 않는다. 본문과 링크는 연출 중에도 사용할 수 있다. 화면 이탈이나 GPU 복구로 등장 연출을 처음부터 다시 시작하지 않는다.

정상 로딩은 첫 윤곽선 이미지, 고정된 실제 3D 윤곽선, 등장 연출 순서다. 일반적인 PC와 세로 모바일의 첫 프레임을 WebP로 저장한다. 가로 화면이나 작은 세로 화면처럼 다른 카메라 구도를 사용하는 경우에는 단색 배경에서 실제 윤곽선으로 전환한다. 완성된 풍경 사진을 정상 로딩의 시작 화면으로 사용하지 않는다. 재질을 비동기로 받는 동안 윤곽선만 먼저 그리며 프레임 루프와 포인터 반응을 시작하지 않는다. 재질이 준비되면 동일한 장면의 진행률 0에서 연출을 이어간다. 기존 사진은 JavaScript 미지원, 모션 감소, 그래픽 오류 시 정적 대체 화면으로만 사용한다.

자동 움직임과 포인터 반응은 따로 보간하고 더 큰 값을 블록에 적용한다. 아래쪽 블록은 고정하고 위쪽 블록은 위치별 위상 차이로 조용히 움직인다. 포인터 반응은 자동 움직임보다 강하며 포인터가 떠나면 자동 움직임만 남는다. 이름의 실제 글자 영역을 호버하면 250ms 동안 작은 수평 변위와 잘린 글자 조각으로 글리치를 표현한다. 이 효과는 글자 크기나 문서 배치를 바꾸지 않으며, 모션 감소 설정에서는 모든 비필수 모션을 생략한다.
