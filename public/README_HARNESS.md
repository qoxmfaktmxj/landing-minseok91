...
> **문서 기준 릴리즈**: v1.14.0 (2026-06-30) — **IBSheetHTML V7 API 레퍼런스 스킬**(`ehr-ibsheet-api`) 추가: 개발자가이드 PDF 에서 메서드 322·이벤트 67·Config 속성 133 = **522 항목**을 verbatim 시그니처로 인덱스화 (유료 솔루션이라 LLM 이 메서드명을 추측하던 문제 해결). v1.13 **DB 무접속 정책**(AI 직접 접속 금지, 사용자 export 폴더에서만 명세 조회, 전 프로파일)은 그대로 유지.
...
> **활용 현황 (2026-09-05)**: 2개 팀 30명 이상에게 배포되어 활용됐고, 소속 팀 공식 개발방법론으로 채택됐습니다. 이후에도 버전 업데이트를 진행하고 있습니다. AI TFT는 완료됐으며, 그 성과를 바탕으로 Claude Code 전사 도입이 결정됐습니다. 아래 기술 설명은 2026-06-30 당시의 v1.14.0 문서로 보존합니다.
...
이 marketplace 는 *두 개의 도메인 plugin* 을 호스팅합니다 — *완전 독립*, 같이 설치 가능, 분리 가능.

| Plugin | 도메인 | 설치 명령 | 진입점 |
|---|---|---|---|
| **ehr-harness** | EHR 4/5 일반 (인사/급여/근태/복리후생/평가) — Spring MVC + MyBatis. **+ workup 프로파일**(클라우드 HR MSA) | `/plugin install ehr-harness@ehr-harness` | "하네스 만들어줘" / **"워크업 하네스 만들어줘"** |
| **yjungsan-harness** | 연말정산 (yjungsan) 전용 — JSP 자체 MVC + Oracle PKG 깊은 분석 + 매년 시즌 sync | `/plugin install yjungsan-harness@ehr-harness` | "연말정산 X" |

> **workup(v1.11.0+)**: 별도 플러그인이 아니라 `ehr-harness` 안의 **프로파일**이다(`profiles/workup/`). 멀티레포 MSA·`/proxy`(zuul) 게이트웨이·JSP/Vue2/Vue3 혼재·서비스별 벤더(Oracle/MySQL)를 다룬다. 전용 진입 스킬 `ehr-harness-workup`("워크업 하네스 만들어줘")이 `PROFILE=workup` 으로 생성한다. DB 는 `application-local.yml`(개발) 전용 — 운영 yml 미접속.

분리 원칙:
- skills/agents/hooks/commands 모두 *완전 독립* (충돌 0)
- 같은 repo 에 살 뿐
- 공유 가능 자원 = `db-read-only.sh` (같은 Oracle 안전장치)

상세 yjungsan-harness 안내: [`plugins/yjungsan-harness/README.md`](plugins/yjungsan-harness/README.md)
...
| **도메인 지식** | 권한 모델, 법칙 판정, 화면 패턴 등 | `.claude/skills/ehr-domain-knowledge/`, `.claude/skills/ehr-lessons/` |
...
│   ├── hooks/
│   │   ├── db-read-only.sh           ← Claude Code Bash SQL CLI **전면 차단 (v1.13+ DB 무접속 — sqlplus/tbsql/mysql/sqlcmd/impdp 등 모든 접속 도구 차단)**. 명세는 사용자 export 폴더에서만
│   │   ├── vcs-no-commit.sh          ← Git/SVN 커밋·푸시 차단 (AI는 코드 수정까지만)
│   │   ├── user-prompt-capture.sh    ← UserPromptSubmit 훅: redaction 후 .claude/learnings/pending.jsonl 캡처 (v1.10+, capture_enabled gate)
│   │   └── session-start-inject.sh   ← SessionStart 훅: CLAUDE_ENV_FILE + additionalContext (≤1KB) 주입 (v1.10+)
│   ├── agents/
│   │   ├── screen-builder.md     ← 화면 생성 에이전트
│   │   ├── procedure-tracer.md   ← 프로시저 체인 분석 에이전트
│   │   ├── release-reviewer.md   ← 릴리즈 검증 + 회귀 판정 (EHR4: B1~B7 / EHR5: B1~B11)
...
│   └── skills/
│       ├── ehr-screen-builder/SKILL.md      ← 화면 생성 코드 템플릿
│       ├── ehr-codebase-navigator/SKILL.md  ← 코드 경로 탐색
│       ├── ehr-procedure-tracer/SKILL.md    ← 프로시저 추적 로직
│       ├── ehr-impact-analyzer/SKILL.md     ← 변경 전 영향도 예측 + Go/HOLD/STOP 판정
│       ├── ehr-db-query/SKILL.md            ← DB 명세 조회 (무접속 — export 폴더 grep/read)
│       ├── ehr-domain-knowledge/SKILL.md    ← 도메인 지식 레퍼런스 (EHR-COMPOUND 블록 누적)
│       ├── ehr-lessons/SKILL.md             ← self-evolving 승급 lesson 저장소 (v1.10+, project-local scope)
│       ├── ehr-ibsheet-api/                 ← IBSheetHTML V7 API 레퍼런스 (v1.14+, 522 항목 인덱스 + 원문 PDF 동봉)
│       └── ehr-design-guide/SKILL.md        ← IDS 컴포넌트 가이드 (Storybook static 빌드 존재 시에만 생성: `src/main/resources/static/guide/storybook-static/`)
│
...
> **DB 무접속 정책 (v1.13+)**: 이 하네스의 AI 는 DB 에 직접 접속하지 않는다. 함수/프로시저/패키지/테이블 명세는 사용자가 폴더로 export 한 뒤 그 경로를 알려주면 AI 가 폴더만 읽는다. db-read-only 훅이 모든 SQL CLI 접속을 차단한다.
...
위 명령을 한 번에 긁어 실행하면 된다. `superpowers` 는 **필수** (옵션 아님) — `/ehr:plan` 이 `superpowers:writing-plans` 를, `/ehr:work` 가 `superpowers:executing-plans` 를, `/ehr:review` 가 `superpowers:receiving-code-review` 를 내부에서 호출하므로 없으면 3개 커맨드가 동작하지 않는다.

> **marketplace 선택 — 어느 쪽이든 통과**: 플러그인의 의존성 검사 로직(`Step 0.5`)은 `enabledPlugins[superpowers@*] === true` 와일드카드 매칭이라 `superpowers@superpowers-marketplace`(원작자 obra 운영) 와 `superpowers@claude-plugins-official`(Anthropic 미러) 둘 다 인정한다. 위 명령은 원작자 마켓플레이스 기준이며, Anthropic 미러를 이미 쓰고 있다면 그대로 두면 된다.
...
- (선택) DB 객체 정의 export 폴더 (DDL·프로시저 소스). AI 는 DB 에 접속하지 않음 — 사용자가 export
...
> **버전 경로 주의**: 아래 명령들은 캐시 디렉토리 이름에 버전이 들어간다는 점에 주의한다 (예: `~/.claude/plugins/cache/ehr-harness/ehr-harness/1.10.2/`). 설치된 버전은 와일드카드(`~/.claude/plugins/cache/ehr-harness/ehr-harness/*/`) 또는 `plugin.json::version` 으로 확인한다. 아래 스크립트는 `VERSION=$(ls ...)` 로 자동 탐지하므로 버전 하드코딩이 필요 없다.
...
**workup(클라우드 HR MSA) 프로젝트**는 멀티레포 부모폴더(예: `C:\WORKUP`)에서 전용 트리거를 쓴다:

```
cd /c/WORKUP        # eureka/ + workup-* 서비스들이 보이는 부모폴더
"워크업 하네스 만들어줘"     # → ehr-harness-workup 스킬, PROFILE=workup 고정
```
> DB 는 `application-local.yml`(개발) 전용 — `application-prod.yml` 등 운영 설정은 읽지·접속하지 않는다.
...
[3] 프로젝트 심층 분석 (~15개 분석 명령)
    모듈 맵, 법칙별 사용 수, 세션 변수, 
    프로시저/트리거 목록, DB 명세 export 폴더,
    핵심 화면 코드 패턴 추출
   │
   ▼
[4] 하네스 파일 생성 (기본 ~35개, Storybook 프로젝트는 +4~5개)
    프로파일 기본 틀 + 프로젝트 실측값 주입
    내역: 문서 4 + 설정 2 + 훅 4(DB·VCS·user-prompt-capture·session-start-inject)
        + 에이전트 4 + 스킬 7 × 2복사본(.claude + .agents) + /ehr:* 커맨드 5
        + 보조 커맨드 2(harvest-learnings·onboarding) + HARNESS.json + GEMINI.md + .gitignore
   │
   ▼
[5] 검증
...
| 5 | `.claude/settings.json` | ~45줄 | PreToolUse / UserPromptSubmit / SessionStart 훅 등록. |
| 6 | `.claude/hooks/db-read-only.sh` | ~50줄 | **(v1.13+) DB 무접속 정책** — Claude Code Bash 에서 SQL CLI(sqlplus/tbsql/sqlcmd/mysql/mariadb/psql/impdp/expdp/rman) 호출을 **전면 차단**. AI 는 DB 에 직접 접속·조회하지 않는다. 함수/프로시저/패키지/테이블 명세는 **사용자가 export 한 폴더**(DUMP_INDEX/DB_MAP)에서만 grep/read. 파일 조작(grep/cat)은 통과. 1차 통제는 read-only 계정·네트워크이며 본 훅은 보조 안전망. |
| 7 | `.claude/hooks/vcs-no-commit.sh` | ~30줄 | Git/SVN 커밋·푸시 차단 훅. AI는 코드 수정까지만 수행. |
| 8 | `.claude/hooks/user-prompt-capture.sh` | ~130줄 | **v1.10+** UserPromptSubmit 훅. 사용자 발화의 success/correction 신호 분류 → redaction → `.claude/learnings/pending.jsonl` append. capture 게이트: HARNESS.json `learnings_meta.capture_enabled` + `.gitignore` 등록 모두 충족 시에만 동작. stdout 0 byte 보장. |
| 9 | `.claude/hooks/session-start-inject.sh` | ~80줄 | **v1.10+** SessionStart 훅. project-local preferences (`EHR-PREFERENCES` 블록) 를 1KB 이하 additionalContext 로 주입. `CLAUDE_ENV_FILE` 도 함께 export. |
| 10 | `.claude/agents/screen-builder.md` | ~80줄 | 화면 생성 에이전트 시스템 프롬프트. |
| 11 | `.claude/agents/procedure-tracer.md` | ~150줄 | 프로시저 분석 에이전트. |
| 12 | `.claude/agents/release-reviewer.md` | EHR4 ~180줄 / EHR5 ~300줄 | 릴리즈 검증 + 회귀 판정 에이전트 (**프로파일별 독립**: EHR4 는 B1~B7, EHR5 는 B1~B11 경계면 매트릭스). 도메인 지식은 유사하되 기술 스택이 다르므로 파일 내용이 다름. 감별된 auth_model/db_verification에 따라 조건부 체크. |
| 13 | `.claude/agents/db-impact-reviewer.md` | ~110줄 | **v1.9.0+** DB 비즈니스 영향 검토 에이전트. 근태/급여 프로시저 변경 시 리뷰 게이트. `{{CRITICAL_PROCS}}` 치환으로 프로젝트별 프로시저 이름 반영. |
| 14 | `.claude/skills/ehr-domain-knowledge/SKILL.md` | ~400-750줄 | 도메인 지식 레퍼런스. EHR-COMPOUND 마커 블록 누적 대상. |
| 15 | `.claude/skills/ehr-screen-builder/SKILL.md` | ~400줄 | 화면 생성 코드 템플릿. |
| 16 | `.claude/skills/ehr-codebase-navigator/SKILL.md` | ~200줄 | 코드 경로 탐색. |
| 17 | `.claude/skills/ehr-procedure-tracer/SKILL.md` | ~300줄 | 프로시저 추적. |
| 18 | `.claude/skills/ehr-db-query/SKILL.md` | ~200줄 | DB 명세 조회 가이드 (무접속 — export 폴더 grep/read). |
| 19 | `.claude/skills/ehr-impact-analyzer/SKILL.md` | ~500줄 | 변경 전 영향도 예측 + HOLD·STOP/HOLD/Conditional-Go/Go 판정 + CHECK 병기. |
| 20 | `.claude/skills/ehr-lessons/SKILL.md` | ~150줄 | **v1.10+** self-evolving 승급 lesson 저장소. `/ehr-harvest-learnings` 가 staged → audit 승인 → 본 파일에 EHR-PROMOTED 블록으로 merge. project-local scope. |
| 21 | `.claude/skills/ehr-design-guide/SKILL.md` | ~200줄 | IDS 컴포넌트 가이드. **조건부 생성** — 프로젝트에 Storybook static 빌드 (`src/main/resources/static/guide/storybook-static/`) 가 존재할 때만 생성. 없으면 이 파일·카탈로그·MANIFEST 전체 스킵. |
| 22 | `.agents/skills/` | — | 위 스킬들의 Codex 호환 복사본 (생성된 스킬만 복사). |
| 23 | `.codex/config.toml` | ~5줄 | Codex CLI 설정. |
| 24 | `.gitignore` | ~20줄 | 런타임 산출물 제외 + `.claude/learnings/` 캡처 디렉토리 (capture 활성 조건). |
| 25 | `.claude/HARNESS.json` | ~80줄 | **schema v5** 매니페스트. plugin_version, profile, sources/outputs sha256, analysis 스냅샷, ehr_cycle (compounds·promoted·preferences_history·learnings_meta). audit/stamped 모드의 baseline. |
| 26 | `GEMINI.md` | ~10줄 | Gemini CLI 진입 (Heavy: 모든 스킬 @참조). |
| 27 | `.claude/commands/ehr/ideate.md` | ~60줄 | **v1.9.0+** `/ehr:ideate` — 하네스 축적 데이터에서 고임팩트 개선 아이디어 제안 + adversarial filter. |
| 28 | `.claude/commands/ehr/plan.md` | ~70줄 | **v1.9.0+** `/ehr:plan` — `superpowers:writing-plans` 래퍼 + `impact-analyzer`/`db-impact-reviewer` 결과를 EHR 영향도 섹션으로 자동 주입. |
| 29 | `.claude/commands/ehr/work.md` | ~55줄 | **v1.9.0+** `/ehr:work` — `superpowers:executing-plans` 래퍼 + design-guide/screen-builder/db-query 자동 라우팅. git 없는 환경 지원. |
| 30 | `.claude/commands/ehr/review.md` | ~75줄 | **v1.9.0+** `/ehr:review` — `receiving-code-review` ∥ `release-reviewer` ∥ `db-impact-reviewer` 병렬 오케스트레이션. |
| 31 | `.claude/commands/ehr/compound.md` | ~135줄 | **v1.9.0+** `/ehr:compound` — 세션·커밋에서 새 사실/패턴/선호 추출 → L2/L3 마커 블록 및 `EHR-PREFERENCES` 갱신. 6개 플래그(`--promote-to-skill` 등) 지원. |
| 32 | `.claude/commands/ehr/harvest-learnings.md` | ~90줄 | **v1.10+** `/ehr-harvest-learnings` — pending.jsonl 클러스터링 → distinct_sessions/cap 기반 score → staged markdown 생성. nonce 발급으로 audit 승인 흐름과 연결. |
| 33 | `.claude/commands/ehr/onboarding.md` | ~115줄 | **v1.10+** `/ehr:onboarding` — 하네스 첫 사용자를 위한 5분 가이드 (활성화 확인 → 첫 명령 → 스킬 카탈로그 → 작업 사이클 안내). |
| 34 | `.claude/skills/ehr-ibsheet-api/` | SKILL ~50줄 + 인덱스 ~744줄 + PDF 3.7MB | **v1.14+** IBSheetHTML V7 API 레퍼런스. 메서드 322·이벤트 67·Config 133 = 522 항목 verbatim 시그니처 인덱스 + 원문 PDF (deep-dive fallback). `.agents` 복사본은 SKILL+인덱스만 (PDF 제외). |

**프로파일별 차이**: 현재 v1.14 기준으로 EHR4와 EHR5 **스킬/에이전트 구성은 동일**(8개 스킬 + 4개 에이전트). `/ehr:*` 커맨드 5종은 `profiles/shared/commands/ehr/*.md.skel` **공용 1벌**에서 치환자로 분기. `harvest-learnings.md`·`onboarding.md` 는 치환 없이 공용 본문 그대로 복사. 각 파일의 내용은 기술 스택에 따라 다르게 생성된다 (Velocity vs MyBatis, Ant vs Maven 등). 프로파일 디렉토리(`profiles/ehr4/`, `profiles/ehr5/`)는 독립적이므로 향후 한쪽에만 특화 자산이 추가될 수 있다.

**파일 총합 (design-guide 제외 기준)**: 문서 4 + 설정 2 + 훅 4 + 에이전트 4 + 스킬 8 × 2복사본 + ibsheet 레퍼런스 3 (`.claude` 인덱스+PDF, `.agents` 인덱스) + `/ehr:*` 커맨드 5 + 보조 커맨드 2 + `HARNESS.json` 1 = **약 40개**. Storybook 있는 프로젝트는 design-guide 스킬 × 2복사본 + 카탈로그/MANIFEST 포함하여 **+4~5개** 추가 생성.
...
| 에이전트 사용 규칙 | 스폰 조건 | 고정 |
| DB 접근 규칙 | **무접속** — export 폴더만 (SQL CLI 전면 차단 훅) | 고정 |
...
| 추적 알고리즘 | 순방향/역방향 | 고정 |
| 전체 프로시저 목록 | grep CALL + export 폴더 인덱싱 | **실측** |
| 전체 트리거 목록 | grep + export 폴더 | **실측** (폴더 없을 시 빈 목록 + 경고) |
...
### 5-7. db-query SKILL.md — DB 명세 조회 (무접속)

**생성 기준**: 사용자 export 폴더(무접속) 기반. AI 는 DB 에 접속하지 않는다 — `db-read-only` 훅이 SQL CLI 전면 차단.

| 섹션 | 내용 | 고정/실측 |
|------|------|----------|
| 무접속 제약 | SQL CLI 전면 차단 훅 + AI 가이드라인 (접속 금지) | 고정 |
| DB 명세 소스 | 해당 없음(접속 안 함). 벤더는 설정파일 읽기로만 감지 | 고정 |
| OJDBC 드라이버 | 해당 없음(드라이버 불필요 — 접속 안 함) | 고정 |
| export 폴더(DUMP_INDEX) | 사용자가 export 한 DDL·프로시저 소스 폴더 — 1차 조회 소스 | **실측** (폴더 경로 + 파일 인덱스) |
| 공통 grep 패턴 5개 | 프로시저 소스, 테이블 구조 — export 폴더 grep/read | 고정 |
| 변경 SQL 작성 규칙 | 변경 SQL 은 파일로만 작성. 실행은 사용자 직접 | 고정 |
...
**생성 기준**: 프로젝트에 Storybook **빌드된 static 산출물** (`src/main/resources/static/guide/storybook-static/` 고정 경로 + `project.json` + `index.json`) 이 존재할 때만 생성. 소스 `stories/` 디렉토리만으로는 불충분 — 빌드 산출물이 필요하다. 없으면 이 섹션 전체 스킵.
...
| release-reviewer.md | 경계면 매트릭스·회귀 패턴·치명 위험·CHECK 판정 (**프로파일별 독립**: EHR4 B1~B7 / EHR5 B1~B11, auth_model 기반 조건부. v1.9.5 에서 CDATA 매퍼 XML 체크 추가로 매트릭스가 확장됨) | 고정 |
...
#### 3-H. DB 명세 export 폴더 요청 (무접속)
```bash
# AI 는 DB 에 접속하지 않는다. 사용자에게 DB 객체 정의를 폴더로 export 요청 → 폴더 스캔.
# 벤더 감지 — 설정파일 읽기만 (접속 없음)
grep -r "spring.datasource.url" --include="*.properties" --include="*.yml"
grep -r "driverClassName\|jdbcUrl" --include="context*.xml"

# export 폴더 파일 인덱싱 (DUMP_INDEX)
# 사용자가 제공한 export 폴더 내 DDL/소스 파일 목록화
...
1. 인프라 (shared/ → 프로젝트 루트)
   .claude/settings.json ← 그대로 복사 (PreToolUse + UserPromptSubmit + SessionStart 훅 등록)
   .claude/hooks/db-read-only.sh ← 그대로 복사
   .claude/hooks/vcs-no-commit.sh ← 그대로 복사
   .claude/hooks/user-prompt-capture.sh ← 그대로 복사 (v1.10+, self-evolving 캡처)
   .claude/hooks/session-start-inject.sh ← 그대로 복사 (v1.10+, preferences 주입)
   .codex/config.toml ← 그대로 복사
   .gitignore ← 그대로 복사 (.claude/learnings/ 라인 자동 추가)

2. 문서 (skeleton/ + 실측값 → 프로젝트 루트)
   AGENTS.md ← 스켈레톤 + {{MODULE_MAP}}, {{PROC_LIST}} 치환
   CLAUDE.md ← 스켈레톤 + {{TECH_STACK}} 치환 + EHR-ROUTING/EHR-PREFERENCES 마커 블록
   README.md ← 스켈레톤 + 규모 수치 치환

3. 도메인 스킬 (skills/ → .claude/skills/, ehr- prefix 자동 부여)
   ehr-domain-knowledge ← 고정 뼈대 + 실측 수치 주입
   ehr-screen-builder ← 고정 뼈대 + 실제 코드 예시 주입
   ehr-codebase-navigator ← 모듈 맵 실측 주입
   ehr-procedure-tracer ← 프로시저/트리거 목록 주입
   ehr-db-query ← DB 무접속 안내 + export 폴더(DUMP) 경로/모드 주입
   ehr-impact-analyzer ← critical_proc_found 주입 + 위험 신호 룰
   ehr-lessons ← v1.10+ self-evolving 승급 lesson 저장소 (초기 빈 상태)
   ehr-ibsheet-api ← v1.14+ IBSheet V7 API 레퍼런스 (고정 복사, 치환 없음 — 522 항목 인덱스 + PDF)

4. 에이전트 (agents/ → .claude/agents/)
   screen-builder.md ← 프로파일별 고정
   procedure-tracer.md ← 고정 + 치명 목록 주입
   release-reviewer.md ← 프로파일별 독립 (EHR4 B1~B7 / EHR5 B1~B11, 조건부 체크 매트릭스)
   db-impact-reviewer.md ← v1.9.0+ 근태/급여 프로시저 영향 검토 + {{CRITICAL_PROCS}} 치환

5. /ehr:* 커맨드 + 보조 커맨드 (.claude/commands/ehr/)
   ideate / plan / work / review / compound ← 5종 .skel + 치환자 적용
   harvest-learnings / onboarding ← 공용 본문 그대로 복사 (치환 없음)

6. Codex/Gemini 호환
   .agents/skills/ ← .claude/skills/ 전체 복사 (생성된 스킬만)
   GEMINI.md ← @참조 목록 생성

7. HARNESS.json 갱신 (schema v5)
   plugin_version + profile + sources/outputs sha256 + analysis 스냅샷
   + ehr_cycle (compounds·promoted·preferences_history·learnings_meta)
```
...
```
1. 생성 파일 수 확인
   EHR4/EHR5 기본: 약 35개 (문서 4 + 설정 2 + 훅 4 + 에이전트 4 + 스킬 7 × 2복사본 + /ehr:* 커맨드 5 + 보조 커맨드 2 + HARNESS.json + GEMINI.md + .gitignore)
   Storybook static 빌드 존재 프로젝트: +4~5개 (design-guide 스킬 × 2복사본 + INDEX/MANIFEST)
...
4. export 폴더 인덱싱 결과
   "export 폴더 인덱싱 완료: N개 객체 (DDL/소스 파일)"
   또는
   "⚠ export 폴더 없음 → code-only.
    객체 정의 export 후 db-query 로 갱신"
```
...
프로파일은 시스템 세대/아키텍처별 **고정 뼈대**를 제공한다. EHR4·EHR5·workup 은 아키텍처가 다르므로 (Anyframe vs MyBatis, 모놀리식 vs MSA, JSP vs JSP+Vue 등) 별도 프로파일이 필요하다. 생성기 Step 1 이 `PROFILE` 을 자동 감지하고(`SKILL_PREFIX` 로 출력 접두 분기: ehr4/5=`ehr-`, workup=`workup-`), workup 은 전용 스킬 `ehr-harness-workup`("워크업 하네스 만들어줘")으로 강제 진입할 수도 있다.
...
│   ├── skills/   ← 스킬 9개 (skel 6: screen-builder, codebase-navigator, procedure-tracer, db-query, impact-analyzer, design-guide + 고정 3: domain-knowledge, ehr-lessons, ibsheet-api)
│   └── reference/ ← 고정 참조 (CODE_MAP, DB_MAP)
├── ehr5/         ← Spring Boot + MyBatis + Maven
│   ├── skeleton/
│   ├── agents/    ← 4개 (screen-builder, procedure-tracer, release-reviewer는 B1~B11, db-impact-reviewer)
│   ├── skills/    ← 9개 (내용은 MyBatis/Spring Boot 기반, ehr-lessons·ibsheet-api 포함)
│   └── reference/
└── workup/       ← 클라우드 HR MSA (멀티레포, Spring Boot+MyBatis, JSP/Vue2/Vue3 혼재, /proxy(zuul) 게이트웨이)
    ├── skeleton/  ← WORKUP-ROUTING/PREFERENCES 마커, Vue 정식 스택, DB local 전용 정책
    ├── agents/    ← 4개 (release-reviewer는 서비스경계/JSP↔Vue/JWT/분산TX 회귀)
    ├── skills/    ← 8개 (workup-* 접두, domain-knowledge 신규 작성)
    └── reference/ ← CODE_MAP, DB_MAP, **SERVICE_MAP**(서비스별 스택/벤더/치명프로시저)
```
...
| `{{PROC_LIST}}` | 전체 프로시저 목록 | grep CALL + export 폴더 |
| `{{TRIGGER_LIST}}` | 전체 트리거 목록 | grep + export 폴더 |
...
| `{{DB_CONNECTION}}` | 벤더 정보만 (설정파일 읽기). 접속 정보 수집 안 함 | properties/xml 읽기 |
...
| 에이전트 수 | 4개 (db-impact-reviewer 포함) | 4개 (db-impact-reviewer 포함) |
| 금지 기술 | MyBatis 금지 | Anyframe 금지 |
| 배치 크기 | 50건 (Velocity 바인드 변수 한계) | 1000건 (Oracle IN절 한계) |
| domain-knowledge | 744줄 (10 섹션) | 411줄 (7 섹션) |
| 추가 섹션 | B1~B7 경계 (CHECK 판정 + CDATA 매퍼 XML 체크 포함), Velocity 레퍼런스, 릴리즈 리스크 | B1~B11 경계 (B7~B10 매퍼 파일명/cMap/NULL 센티넬/감사 컬럼 + B11 CDATA 매퍼 XML 체크) |
...
- 한국어 에러 메시지
- 그린필드 금지 원칙
- DB 무접속 정책 (SQL CLI 전면 차단 훅 + export 폴더만). 1차 통제는 read-only 계정·네트워크, 본 훅은 보조
...
- 금지 사항 목록
- 그린필드 금지 원칙
- 경계 검증 매트릭스 + CHECK 판정 (**프로파일별**: EHR4 B1~B7, EHR5 B1~B11)
- 에이전트 역할 정의
- db-read-only 훅 + vcs-no-commit 훅 + (v1.10+) user-prompt-capture / session-start-inject 훅
...
│           │   ├── hooks/
│           │   │   ├── db-read-only.sh           # SQL CLI 전면 차단 (v1.13+ DB 무접속 — sqlplus/tbsql/mysql/mysqldump/sqlcmd/isql/osql/psql/sqlldr/impdp/expdp/rman/db2 등 접속 도구 자체를 차단)
│           │   │   ├── vcs-no-commit.sh          # Git/SVN 커밋·푸시 차단
│           │   │   ├── user-prompt-capture.sh    # v1.10+ self-evolving 캡처 (UserPromptSubmit, redaction → pending.jsonl)
│           │   │   └── session-start-inject.sh   # v1.10+ preferences 주입 (SessionStart, ≤1KB additionalContext)
...
│           │   │   ├── review.md.skel
│           │   │   └── compound.md.skel
│           │   ├── skills/ibsheet-api/references/   # v1.14+ IBSheet 레퍼런스 단일 소스 (IBSHEET_API.md 522 항목 + 원문 PDF)
│           │   ├── .codex/
...
│           │   ├── skills/       # 스킬 9개 (6개 .skel + domain-knowledge·ehr-lessons·ibsheet-api 고정본 + design-guide는 대상 프로젝트에 Storybook 있을 때만 생성)
...
│           │   │   ├── ehr-lessons/SKILL.md   # v1.10+ self-evolving 승급 lesson 저장소
│           │   │   └── ibsheet-api/SKILL.md   # v1.14+ IBSheet V7 API 레퍼런스 (본문 인덱스+PDF 는 shared/skills/ibsheet-api/)
│           │   └── reference/    # 고정 참조 문서
...
│               ├── skills/       # 스킬 9개 (EHR4와 동일 구조, 내용은 MyBatis/Spring Boot 기반)
...
| `db_verification` | DDL 폴더 존재 + export 폴더(dump) 유무로 b3_strategy 결정 | B3 검증 Tier 1/2/3 폴백 전략 |
...
- **DB 검증 폴백**: repo DDL 폴더 → export 폴더(dump) → 수동 (3-tier, DB 직접 접속 없음)
...
1. **L2 지식** (`EHR-COMPOUND` 블록) — `reference/CODE_MAP.md` · `DB_MAP.md` · `.claude/skills/ehr-domain-knowledge/SKILL.md` (타깃 프로젝트) / `profiles/<profile>/skills/domain-knowledge/SKILL.md` (플러그인 dev)
...
### HARNESS.json v2 → v3 → v4 → v5 마이그레이션

기존 v2 사용자가 "하네스 점검해줘" 하면:
1. 매니페스트가 legacy 로 분류됨
2. "adopt / 전체 재생성 / 취소" 선택지 제공
3. adopt 선택 시 → 현재 분석 결과로 analysis 생성 + v5 재스탬프 (ehr_cycle + learnings_meta 기본 주입)
4. 다음 audit 부터 정상 drift 계산 작동

v3/v4 사용자는 **legacy 분류되지 않고** 그대로 stamped 상태 유지 (마이그레이션 유예: `HS_SCHEMA_VERSION_STAMPED="3 4 5"`). 다음 재생성 또는 audit 시 `hs_migrate_v4_to_v5()` 가 idempotent 하게 `learnings_meta` 만 strict-append 로 주입하고 `schema_version` 을 5 로 올린다.
...
1. L2 지식 블록 (`EHR-COMPOUND`) — `reference/*`, `ehr-domain-knowledge/SKILL.md`
2. L3 승급 블록 (`EHR-PROMOTED`) — `agents/*.md`, `skills/**/SKILL.md` (`ehr-lessons` 포함)
...
| 보조 커맨드 2종 | `profiles/shared/commands/ehr/harvest-learnings.md`, `onboarding.md` |
| `db-impact-reviewer` 에이전트 | `profiles/ehr4\|ehr5/agents/db-impact-reviewer.md` |
| `ehr-lessons` 스킬 (v1.10+) | `profiles/ehr4\|ehr5/skills/ehr-lessons/SKILL.md` |
| self-evolving 훅 2종 (v1.10+) | `profiles/shared/hooks/user-prompt-capture.sh`, `session-start-inject.sh` |
| 마커 머지 헬퍼 | `skills/ehr-harness/lib/ehr-compound.sh` |
| 학습 라이브러리 (v1.10+) | `skills/ehr-harness/lib/learnings.sh`, `redaction.sh` |
| audit 드리프트 4종 | `skills/ehr-harness/lib/audit.sh` (`ehr_audit_*` 함수군) |
| HARNESS.json v5 | `ehr_cycle` (`compounds[]`·`promoted[]`·`preferences_history[]`·`learnings_meta`) |
| CLAUDE.md 블록 | `EHR-ROUTING` (규칙 고정) + `EHR-PREFERENCES` (선호 누적) |

### 기존 하네스(v3/v4) 마이그레이션

각 업체 프로젝트에서 `/ehr-harness 재생성해줘` 한 번 실행하면 자동으로 v3/v4 → v5 로 승격된다. v3 → v5 는 `ehr_cycle` 섹션 + `learnings_meta` 모두 신규 주입이고, v4 → v5 는 `hs_migrate_v4_to_v5()` 가 strict-append 로 `learnings_meta` 만 추가한다. 업체 간 지식은 **프로젝트-로컬 격리**로 절대 섞이지 않는다.
