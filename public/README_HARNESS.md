...
> **현재 릴리즈**: v1.14.0 (2026-06-30) — **IBSheetHTML V7 API 레퍼런스 스킬**(`ehr-ibsheet-api`) 추가: 개발자가이드 PDF 에서 메서드 322·이벤트 67·Config 속성 133 = **522 항목**을 verbatim 시그니처로 인덱스화 (유료 솔루션이라 LLM 이 메서드명을 추측하던 문제 해결). v1.13 **DB 무접속 정책**(AI 직접 접속 금지, 사용자 export 폴더에서만 명세 조회, 전 프로파일)은 그대로 유지.
