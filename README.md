...
첫 렌더링에서는 PC 2.6초, 터치 기기 1.8초 동안 윤곽선이 얼음 재질로 변하고 카메라가 기본 위치로 내려옵니다. 연결망은 등장 순간에만 보이며 숫자 표시는 없습니다. 이후에는 블록이 서로 다른 속도로 조금씩 움직이고, 포인터 가까이의 블록은 더 크게 반응합니다. 이름에 마우스를 올리면 0.25초의 짧은 글리치 효과가 재생됩니다. 모션 감소 설정에서는 등장과 자동 움직임, 글리치를 생략합니다.
