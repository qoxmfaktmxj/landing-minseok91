...
그래픽 일시 정지와 모션 감소 설정을 지원하며, 탭이 숨겨지거나 첫 화면을 벗어나면 렌더링을 멈춥니다. WebGL을 사용할 수 없거나 컨텍스트가 손실되면 같은 로컬 풍경 이미지가 남습니다. JavaScript 없이도 풍경, 경력과 프로젝트 링크를 확인할 수 있습니다. 최종 시각 규칙은 `DESIGN.md`를 참고하세요.
...
3D 배경과 주요 동작은 Chromium에서 다시 검증할 수 있습니다. 테스트는 production build 후 전용 포트3112에서 실행합니다.
...
Windows에서 실제 GPU 경로를 확인하려면 `PLAYWRIGHT_HARDWARE_GPU=1` 환경 변수를 사용할 수 있습니다. 테스트는 커서 반응, 정지와 재생, 화면 이탈, 모션 감소, WebGL 대체 화면과 모바일 터치 종료를 확인합니다.
